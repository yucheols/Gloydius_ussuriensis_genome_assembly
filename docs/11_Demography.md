...
### Step 13: Demographic estimate using SMC++
For our demographic inference, we will use a mutation rate of 1.25 × 10e−8 per site per generation and a generation time of 3 years. Only the mutation rate actually goes into SMC++; generation time is then used to plot changes in Ne through time.
...
After this, we can use smc++ plot command to generate a plot. This operation is lightweight and can be run on the head node.
```sh
# set directory
WORKDIR="/home/yshin/mendel-nas1/snake_genome_ass/G_ussuriensis_Chromo/demography"
SIF="${WORKDIR}/00_env/smcpp_latest.sif"
...
# run smc++ to generate a .csv file
apptainer exec \
    --cleanenv \
    --bind "${WORKDIR}:/work" \
    "$SIF" \
